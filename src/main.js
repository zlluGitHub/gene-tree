      ...
      if (phylotree.radial()) {
        ...
        nodes.forEach(function (d) {
          ...
          if (d3_phylotree_is_leafnode(d)) {
            var bubble_size = d.y + phylotree.node_bubble_size(d)
        ...
        if (!d3_phylotree_is_leafnode(node)) {
          ...
          if (options["selectable"]) {
            ...
            menu_object
              ...
              .append("a")
              .attr("tabindex", "-1")
              .text(optlan == 'chinese' ? "选择叶端分支" : "All terminal branches")
              .on("click", function (d) {
                menu_object.style("display", "none");
                phylotree.modify_selection(
            ...
            menu_object
              ...
              .append("a")
              .attr("tabindex", "-1")
              .text(optlan == 'chinese' ? "选择内部树分支" : "All internal branches")
              .on("click", function (d) {
                menu_object.style("display", "none");
                phylotree.modify_selection(
        ...
        if (node.parent) {
          if (options["selectable"]) {
            menu_object
              ...
              .append("a")
              .attr("tabindex", "-1")
              .text(optlan == 'chinese' ? "当前树分支" : "Incident branch")
              .on("click", function (d) {
                menu_object.style("display", "none");
            ...
            menu_object
              ...
              .append("a")
              .attr("tabindex", "-1")
              .text(optlan == 'chinese' ? "此节点到根路径" : "Path to root")
              .on("click", function (d) {
                menu_object.style("display", "none");
          ...
          if (options['reroot']) {
            menu_object
              ...
              .append("a")
              .attr("tabindex", "-1")
              .text(optlan == 'chinese' ? "在此处重新生根布局" : "Reroot on this node")
              .on("click", function (d) {
                menu_object.style("display", "none");
          ...
          if (options['hide']) {
            menu_object
              ...
              // )
              .text(optlan == 'chinese' ? "隐藏子代树分支" : "Hide this subtree")
              .on("click", function (d) {
                menu_object.style("display", "none");
                phylotree
                  .modify_selection([node], "notshown", true, true, "true")
          ...
          if (options['add']) {
            menu_object
              ...
              //   (d3_phylotree_is_leafnode(node) ? "node" : "subtree")
              // )
              .text(optlan == 'chinese' ? "添加新分支" : "Add this subtree")
              .on("click", function (d) {

                var mark = node.name + ":" + node.attribute;
                var before = inData.slice(0, inData.indexOf(mark))
                var after = inData.slice(inData.indexOf(mark) + mark.length, inData.length)
                // console.log(node);
                // console.log(mark);
                // console.log(before);
                // console.log(after);
                ...
                $('.modal-button .ok').on("click", function (e) {
                  ...
                  if (!inputName) {
                    ...
                    inData = before + mark + "," + inputName + ":" + inputLength + after;
                    init_tree(inData);
    ...
    phylotree.draw_node = function (container, node, transitions) {
      ...
      if (d3_phylotree_is_leafnode(node)) {
        ...
        if (radius > 0) {
          ...
          ele.enter().append("text").attr("dx", options.itemStyle.node.font.x).attr("dy", options.itemStyle.node.font.y).attr("style", 'font-size: ' + options.itemStyle.node.font.size + 'px; fill: ' + options.itemStyle.node.font.color + '; font-family: ' + options.itemStyle.node.font.family + ';').text(function (d) { return d.name });
