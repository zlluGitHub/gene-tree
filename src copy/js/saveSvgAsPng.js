...
out$.svgAsPngUri = (el, options, done) => {
  ...
  const convertToPng = ({ src, width, height }) => {
    ...
    //是否输出pdf 否则输出 其他格式
    if (out$.isPdf) {
      let pdf = new jsPDF(out$.pdfSpecs[0], out$.pdfSpecs[1], out$.pdfSpecs[2]);
      pdf.addImage(png, 'PNG', out$.pdfSize[0], out$.pdfSize[1], out$.pdfSize[2], out$.pdfSize[3]);
...
out$.saveSvgAsPdf = (el, name, options) => {
  out$.isPdf = true;
  out$.fileName = name;
  out$.pdfSpecs = options.pdfSpecs;
  out$.pdfSize = options.pdfSize;
  return requireDomNodePromise(el)
    .then(el => out$.svgAsPngUri(el, options || {}))
};